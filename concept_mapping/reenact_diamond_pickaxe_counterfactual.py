...
def _cover_visible_diamond(obs: np.ndarray, replacement: BlockType) -> np.ndarray:
    out = obs.copy()
...
def _remove_iron_pickaxe(obs: np.ndarray) -> np.ndarray:
    out = obs.copy()
    out[MAP_DIM + IRON_PICKAXE_INDEX] = 0.0
    return out
...
def _compose(cells: dict[str, Image.Image], values: dict[str, float], out_path: str, note: str) -> None:
    row_label_w = 136
    col_header_h = 44
    foot_h = 112
    gap = 14
    keys = ("actual", "diamond_covered", "no_pickaxe", "no_pickaxe_covered")
    ...
    draw.text((row_label_w + 8, 12), "Diamond visible", fill=(0, 0, 0))
    draw.text((row_label_w + cell_w + gap + 8, 12), "Diamond -> stone", fill=(0, 0, 0))
    draw.text((10, col_header_h + cell_h // 2 - 8), "Actual inventory", fill=(0, 0, 0))
    draw.text((10, col_header_h + cell_h + gap + cell_h // 2 - 8), "No iron pickaxe", fill=(0, 0, 0))

    positions = {
        "actual": (row_label_w, col_header_h),
        "diamond_covered": (row_label_w + cell_w + gap, col_header_h),
        "no_pickaxe": (row_label_w, col_header_h + cell_h + gap),
        "no_pickaxe_covered": (row_label_w + cell_w + gap, col_header_h + cell_h + gap),
    }
    for key, pos in positions.items():
        sheet.paste(cells[key], pos)

    diamond_effect_with_pickaxe = values["actual"] - values["diamond_covered"]
    diamond_effect_no_pickaxe = values["no_pickaxe"] - values["no_pickaxe_covered"]
    pickaxe_effect_with_diamond = values["actual"] - values["no_pickaxe"]
    pickaxe_effect_without_diamond = values["diamond_covered"] - values["no_pickaxe_covered"]
    interaction = diamond_effect_with_pickaxe - diamond_effect_no_pickaxe

    y = col_header_h + 2 * cell_h + gap + 10
    for i, line in enumerate(note.split("\n")):
        draw.text((8, y + 16 * i), line, fill=(0, 0, 0))
    draw.text((8, y + 38), f"Diamond effect with iron pickaxe: {diamond_effect_with_pickaxe:.4f}", fill=(0, 0, 0))
    draw.text((8, y + 56), f"Diamond effect no iron pickaxe:   {diamond_effect_no_pickaxe:.4f}", fill=(0, 0, 0))
    draw.text((8, y + 74), f"Iron pickaxe effect with diamond: {pickaxe_effect_with_diamond:.4f}", fill=(0, 0, 0))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Re-enact a visible-diamond rollout state and probe iron-pickaxe/diamond value interaction."
    )
    parser.add_argument("--run_path", default="wandb/run-20260528_225318-5v1h672r")
    parser.add_argument("--out_dir", default="concept_mapping/runs/diamond_pickaxe_reenact_episode06")
    parser.add_argument("--timestep", type=int, default=None)
    parser.add_argument("--env_name", type=str, default=None)
    parser.add_argument("--seed", type=int, default=1)
    parser.add_argument("--episode", type=int, default=6)
    parser.add_argument("--step", type=int, default=304)
    ...
    diamond_covered = _cover_visible_diamond(actual, BlockType.STONE)
    no_pickaxe = _remove_iron_pickaxe(actual)
    no_pickaxe_covered = _cover_visible_diamond(no_pickaxe, BlockType.STONE)

    obs_by_key = {
        "actual": actual,
        "diamond_covered": diamond_covered,
        "no_pickaxe": no_pickaxe,
        "no_pickaxe_covered": no_pickaxe_covered,
    }
    values = {key: _value(network, train_state.params, value) for key, value in obs_by_key.items()}

    csv_path = os.path.join(args.out_dir, "diamond_pickaxe_reenact_values.csv")
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["cell", "iron_pickaxe_present", "diamond_visible", "value"])
        writer.writerow(["actual", True, True, values["actual"]])
        writer.writerow(["diamond_covered", True, False, values["diamond_covered"]])
        writer.writerow(["no_pickaxe", False, True, values["no_pickaxe"]])
        writer.writerow(["no_pickaxe_covered", False, False, values["no_pickaxe_covered"]])
        writer.writerow([])
        writer.writerow(["diamond_effect_with_iron_pickaxe", values["actual"] - values["diamond_covered"]])
        writer.writerow(["diamond_effect_no_iron_pickaxe", values["no_pickaxe"] - values["no_pickaxe_covered"]])
        writer.writerow(["iron_pickaxe_effect_with_diamond", values["actual"] - values["no_pickaxe"]])
        writer.writerow(["iron_pickaxe_effect_without_diamond", values["diamond_covered"] - values["no_pickaxe_covered"]])
        writer.writerow([
            "interaction",
            (values["actual"] - values["diamond_covered"])
            - (values["no_pickaxe"] - values["no_pickaxe_covered"]),
        ])
        ...
        writer.writerow(["actual_summary", repr(_summary(actual))])
    ...
    fig_path = os.path.join(args.out_dir, "diamond_pickaxe_reenact_grid.png")
    note = (
    ...
    _compose(cells, values, fig_path, note)
    ...
    for key in ("actual", "diamond_covered", "no_pickaxe", "no_pickaxe_covered"):
        print(f"{key}: V={values[key]:.4f}", flush=True)
    print(
        f"interaction={(values['actual'] - values['diamond_covered']) - (values['no_pickaxe'] - values['no_pickaxe_covered']):.4f}",
        flush=True,
    )
