"""
M5 Model-Based RL (Dyna + Patch NNT tokenizer + Block Teacher Forcing) for Craftax
...
M5 (M4 + Block Teacher Forcing):
- From step 0: Collect data, build/refresh patch nearest-neighbor tokenizer codebook
- Train TWM from tokenizer tokens with block-causal attention + hybrid BTF supervision
...
- Tokenizer is patch-factorized nearest-neighbor (9x9 patches of size 7x7 -> 81 tokens/frame)

Paper's training counts per iteration (M5):
...
# World model modules
from token_wm.tokenizer.patch_nnt import (
    PatchNNT,
    extend_nnt_codebook_from_obs,
    set_nnt_codebook_params,
)
...
def create_twm(config, *, attention=None, obs_loss_mode=None, ar_aux_weight=None):
    twm_config = TransformerConfig(
        ...
        attention=attention if attention is not None else config["TWM_TRAIN_ATTENTION"],
    ...
    return WorldModel(
        ...
        obs_loss_mode=obs_loss_mode if obs_loss_mode is not None else config["TWM_OBS_LOSS_MODE"],
        ar_aux_weight=float(ar_aux_weight if ar_aux_weight is not None else config["TWM_AR_AUX_WEIGHT"]),
    )
...
def _get_nnt_codebook_count(params) -> int:
    try:
        if isinstance(params, dict):
            return int(np.asarray(params["params"]["codebook_count"]))
        return int(np.asarray(unfreeze(params)["params"]["codebook_count"]))
    except Exception:
        return 0
...
def create_vqvae_train_state(config, rng, sample_obs):
    ...
    # NNT tokenizer is non-parametric; keep a no-op optimizer so params remain checkpointable.
...
def make_twm_update_fn(twm, vqvae, config):
    ...
    def _twm_loss_fn(params, obs_tokens, actions, rewards, dones, dropout_rng):
        ...
        return loss_output.total_loss, (
            loss_output.loss_obs,
            loss_output.loss_obs_btf,
            loss_output.loss_obs_ar,
            loss_output.loss_rewards,
            loss_output.loss_ends,
        )
    ...
    def twm_update_single(twm_state, vqvae_params, obs_batch, actions_batch, rewards_batch, dones_batch, rng):
        ...
        (loss, (loss_obs, loss_obs_btf, loss_obs_ar, loss_rew, loss_ends)), grads = grad_fn(
        ...
        return twm_state, loss, (loss_obs, loss_obs_btf, loss_obs_ar, loss_rew, loss_ends), rng
...
def load_checkpoint(ckpt_path, config, network, vqvae, twm):
    ...
    vq_opt_path = os.path.join(ckpt_path, "vqvae_opt_state.pkl")
    if os.path.exists(vq_opt_path):
        with open(vq_opt_path, "rb") as f:
...
def run_mbrl(config):
    """Run M5 MBRL training (Dyna + patch NNT tokenizer + block teacher forcing)."""
    ...
    config["NNT_TAU_SQ"] = float(config.get("NNT_TAU_SQ", 0.75))
    config["NNT_BUILD_FRAMES"] = max(1, int(config.get("NNT_BUILD_FRAMES", 4096)))
    config["NNT_REFRESH_EVERY"] = max(1, int(config.get("NNT_REFRESH_EVERY", 1)))
    config["NNT_REFRESH_AFTER_BP_ONLY"] = bool(config.get("NNT_REFRESH_AFTER_BP_ONLY", False))
    config["NNT_FREEZE_AFTER_STEP"] = int(config.get("NNT_FREEZE_AFTER_STEP", -1))
    ...
    config["TWM_TRAIN_ATTENTION"] = str(config.get("TWM_TRAIN_ATTENTION", "block_causal"))
    config["TWM_DECODE_ATTENTION"] = str(config.get("TWM_DECODE_ATTENTION", "causal"))
    config["TWM_OBS_LOSS_MODE"] = str(config.get("TWM_OBS_LOSS_MODE", "hybrid_btf_ar"))
    config["TWM_AR_AUX_WEIGHT"] = max(0.0, float(config.get("TWM_AR_AUX_WEIGHT", 0.25)))
    if config["TWM_TRAIN_ATTENTION"] not in ("causal", "block_causal"):
        raise ValueError(f"Unsupported TWM_TRAIN_ATTENTION={config['TWM_TRAIN_ATTENTION']}")
    if config["TWM_DECODE_ATTENTION"] not in ("causal", "block_causal"):
        raise ValueError(f"Unsupported TWM_DECODE_ATTENTION={config['TWM_DECODE_ATTENTION']}")
    if config["TWM_OBS_LOSS_MODE"] not in ("autoregressive", "block_teacher_forcing", "hybrid_btf_ar"):
        raise ValueError(f"Unsupported TWM_OBS_LOSS_MODE={config['TWM_OBS_LOSS_MODE']}")
    ...
    if config["USE_WANDB"]:
        wandb.init(
            ...
            config=config,
            name=f"M5-BTF-{config['ENV_NAME']}-{int(config['TOTAL_TIMESTEPS']//1e6)}M",
        )

    print("\n" + "="*70)
    print("M5 MBRL Training (Dyna + Patch NNT + Block Teacher Forcing)")
    print("="*70)
    ...
    print("-"*70)
    print(f"NNT codebook size (max): {config['VQVAE_CODEBOOK_SIZE']}")
    print(f"NNT tau^2: {config['NNT_TAU_SQ']}")
    print(f"NNT build frames: {config['NNT_BUILD_FRAMES']}")
    print(f"NNT refresh every outer updates: {config['NNT_REFRESH_EVERY']}")
    print(f"NNT refresh only after T_BP: {config['NNT_REFRESH_AFTER_BP_ONLY']}")
    print(f"NNT freeze after step: {config['NNT_FREEZE_AFTER_STEP']}")
    ...
    print(f"TWM train attention: {config['TWM_TRAIN_ATTENTION']}")
    print(f"TWM decode attention: {config['TWM_DECODE_ATTENTION']}")
    print(f"TWM obs loss mode: {config['TWM_OBS_LOSS_MODE']} (ar_aux_weight={config['TWM_AR_AUX_WEIGHT']})")
    ...
    print("-"*70)
    print(f"Pre-built tokenizer params: {config['USE_PRETRAINED_TOKENIZER']}")
    ...
    vqvae = create_vqvae(config)
    train_twm = create_twm(
        config,
        attention=config["TWM_TRAIN_ATTENTION"],
        obs_loss_mode=config["TWM_OBS_LOSS_MODE"],
        ar_aux_weight=config["TWM_AR_AUX_WEIGHT"],
    )
    # Keep AR imagination decode stable by using causal attention at generation time.
    decode_twm = create_twm(
        config,
        attention=config["TWM_DECODE_ATTENTION"],
        obs_loss_mode="autoregressive",
        ar_aux_weight=config["TWM_AR_AUX_WEIGHT"],
    )
    ...
    print(f"NNT tokenizer state size: {vqvae_param_count:,} values")
    
    # Load pre-built tokenizer params if specified
    if config["USE_PRETRAINED_TOKENIZER"]:
        print(f"Loading pre-built NNT tokenizer from: {config['TOKENIZER_PATH']}")
        with open(config["TOKENIZER_PATH"], 'rb') as f:
            loaded_params = pickle.load(f)
        vqvae_state = vqvae_state.replace(params=loaded_params)
        print(f"NNT tokenizer loaded successfully! codebook_count={_get_nnt_codebook_count(vqvae_state.params)}")
    ...
    # Flat buffer for tokenizer sampling/imagination starting states
    ...
    if config.get("RESUME_FROM"):
        ...
        policy_state, vqvae_state, twm_state, fbx_buffer_state, buffer_data, metadata, rollout_state = load_checkpoint(
            config['RESUME_FROM'], config, network, vqvae, train_twm
        )
    ...
    twm_update_single = make_twm_update_fn(train_twm, vqvae, config)
    real_policy_step = make_real_policy_update_fn(network, config)
    imagination_step = make_imagination_fn(network, vqvae, decode_twm, config)
    update_buffer = make_buffer_update_fn(config)

    def refresh_nnt_codebook(vq_state, obs_seed_batch, rng_key):
        """Append-only NNT codebook growth from new observations."""
        if obs_seed_batch is None or int(obs_seed_batch.shape[0]) <= 0:
            return vq_state, _get_nnt_codebook_count(vq_state.params), 0, rng_key

        take = min(int(obs_seed_batch.shape[0]), int(config["NNT_BUILD_FRAMES"]))
        rng_key, idx_rng = jax.random.split(rng_key)
        idx = jax.random.choice(idx_rng, int(obs_seed_batch.shape[0]), shape=(take,), replace=False)
        obs_sample = np.asarray(obs_seed_batch[idx])

        if isinstance(vq_state.params, dict):
            codebook_np = np.asarray(vq_state.params["params"]["codebook"], dtype=np.float32)
            old_count = int(np.asarray(vq_state.params["params"]["codebook_count"]))
        else:
            p_mut = unfreeze(vq_state.params)
            codebook_np = np.asarray(p_mut["params"]["codebook"], dtype=np.float32)
            old_count = int(np.asarray(p_mut["params"]["codebook_count"]))

        codebook_np, new_count, added = extend_nnt_codebook_from_obs(
            codebook_np,
            old_count,
            obs_sample,
            patch_size=int(config["PATCH_SIZE"]),
            tau_sq=float(config["NNT_TAU_SQ"]),
        )
        new_params = set_nnt_codebook_params(vq_state.params, codebook_np, int(new_count))
        vq_state = vq_state.replace(params=new_params)
        return vq_state, int(new_count), int(added), rng_key
    ...
    for update_idx in pbar:
        ...
        # ---------------------------------------------------------------------
        # Step 2: Update Buffers (flat for tokenizer/imagination, flashbax for TWM)
        # ---------------------------------------------------------------------
        # Flat buffer update for tokenizer and imagination starting states
        ...
        # ---------------------------------------------------------------------
        # Step 3: NNT codebook refresh (non-parametric tokenizer; no optimizer)
        # ---------------------------------------------------------------------
        vqvae_metrics = {
            "total_loss": 0.0,
            "l1": 0.0,
        ...
        nnt_codebook_size = _get_nnt_codebook_count(vqvae_state.params)
        nnt_new_codes = 0
        nnt_refreshed = 0
        nnt_refresh_enabled = (
            (not config["USE_PRETRAINED_TOKENIZER"])
            and (
                (config["NNT_FREEZE_AFTER_STEP"] < 0)
                or (total_steps < config["NNT_FREEZE_AFTER_STEP"])
            )
            and (
                (not config["NNT_REFRESH_AFTER_BP_ONLY"])
                or (total_steps >= config["BACKGROUND_PLANNING_START"])
            )
        )
        if nnt_refresh_enabled and ((update_idx - start_update) % config["NNT_REFRESH_EVERY"] == 0):
            vqvae_state, nnt_codebook_size, nnt_new_codes, rng = refresh_nnt_codebook(
                vqvae_state, traj_obs, rng
            )
            nnt_refreshed = 1
        else:
            nnt_codebook_size = _get_nnt_codebook_count(vqvae_state.params)
        ...
        if can_sample_twm and config["N_ITERS_TWM"] > 0:
            for twm_iter in range(config["N_ITERS_TWM"]):
                for mb in range(config["N_MB_WM"]):
                    ...
                    twm_state, twm_loss, (twm_loss_obs, twm_loss_obs_btf, twm_loss_obs_ar, twm_loss_rew, twm_loss_ends), rng = twm_update_single(
        ...
        if True:
            ...
            pbar.set_postfix({
                ...
                'nnt_k': f'{int(nnt_codebook_size)}',
                'nnt_Δ': f'{int(nnt_new_codes)}',
                'twm': f'{float(twm_loss):.3f}',
                'twm_ob': f'{float(twm_loss_obs_btf):.3f}',
                'twm_oa': f'{float(twm_loss_obs_ar):.3f}',
            ...
            if config["USE_WANDB"]:
                log_dict.update({
                    'step': total_steps,
                    'nnt_codebook_size': int(nnt_codebook_size),
                    'nnt_new_codes': int(nnt_new_codes),
                    'nnt_refreshed': int(nnt_refreshed),
                    'twm_loss': float(twm_loss),
                    'twm_loss_obs': float(twm_loss_obs),
                    'twm_loss_obs_btf': float(twm_loss_obs_btf),
                    'twm_loss_obs_ar': float(twm_loss_obs_ar),
    ...
    if config["SAVE_POLICY"]:
        save_dir = f"checkpoints/m5_mbrl_{config['ENV_NAME']}_{config['SEED']}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="M5 MBRL for Craftax (Dyna + Patch NNT + Block Teacher Forcing)")
    ...
    # NNT tokenizer
    parser.add_argument("--vqvae_codebook_size", type=int, default=4096,
                        help="Max NNT codebook size K (paper M4/M5: 4096)")
    parser.add_argument("--vqvae_embed_dim", type=int, default=128,
                        help="Unused for NNT (kept for compatibility)")
    ...
    parser.add_argument("--use_pretrained_tokenizer", action="store_true", 
                        help="Load pre-built NNT tokenizer params (codebook + count) and skip codebook refresh")
    parser.add_argument("--tokenizer_path", type=str, default="token_wm/tokenizer/nnt_params.pkl",
                        help="Path to pre-built NNT tokenizer params")
    ...
    parser.add_argument("--tokenizer_stop_after_step", type=int, default=-1,
                        help="Unused for NNT (kept for compatibility)")
    parser.add_argument("--nnt_tau_sq", type=float, default=0.75,
                        help="NNT squared-L2 threshold tau in Eq.(1) (paper: 0.75)")
    parser.add_argument("--nnt_build_frames", type=int, default=4096,
                        help="Replay frames sampled when rebuilding NNT codebook")
    parser.add_argument("--nnt_refresh_every", type=int, default=1,
                        help="Rebuild NNT codebook every N outer updates")
    parser.add_argument("--nnt_refresh_after_bp_only", action=argparse.BooleanOptionalAction, default=False,
                        help="If true, delay NNT codebook refresh until background_planning_start")
    parser.add_argument("--nnt_freeze_after_step", type=int, default=-1,
                        help="If >=0, stop NNT codebook refresh once total_steps reaches this value")
    ...
    parser.add_argument("--twm_train_attention", type=str, default="block_causal",
                        choices=["causal", "block_causal"],
                        help="Attention pattern for TWM training.")
    parser.add_argument("--twm_decode_attention", type=str, default="causal",
                        choices=["causal", "block_causal"],
                        help="Attention pattern for TWM imagination decode.")
    parser.add_argument("--twm_obs_loss_mode", type=str, default="hybrid_btf_ar",
                        choices=["autoregressive", "block_teacher_forcing", "hybrid_btf_ar"],
                        help="Observation objective for TWM training.")
    parser.add_argument("--twm_ar_aux_weight", type=float, default=0.25,
                        help="Auxiliary AR observation loss weight for hybrid_btf_ar mode.")
    ...
    if args.smoke_test:
        print("\n*** SMOKE TEST MODE (M5 patch NNT + BTF) ***\n")
        ...
        if os.path.exists(args.tokenizer_path):
            ...
            print(f"Tokenizer not found at {args.tokenizer_path}; smoke test will build NNT codebook from replay.")
        ...
        args.nnt_build_frames = 1024
        args.nnt_refresh_every = 1
