"""
M4 Model-Based RL (Dyna + Patch NNT tokenizer) for Craftax
...
- Train TWM from tokenizer tokens
...
Paper's training counts per iteration (M4):
...
def run_mbrl(config):
    """Run M4 MBRL training (Dyna + patch nearest-neighbor tokenizer)."""
    ...
    if config["USE_WANDB"]:
        wandb.init(
            ...
            config=config,
            name=f"M4-NNT-{config['ENV_NAME']}-{int(config['TOTAL_TIMESTEPS']//1e6)}M",
        )

    print("\n" + "="*70)
    print("M4 MBRL Training (Dyna + Patch Nearest-Neighbor Tokenizer)")
    print("="*70)
    ...
    if config["SAVE_POLICY"]:
        save_dir = f"checkpoints/m4_mbrl_{config['ENV_NAME']}_{config['SEED']}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="M4 MBRL for Craftax (Dyna + Patch Nearest-Neighbor Tokenizer)")
    ...
    parser.add_argument("--vqvae_codebook_size", type=int, default=4096,
                        help="Max NNT codebook size K (paper M4: 4096)")
    ...
    if args.smoke_test:
        print("\n*** SMOKE TEST MODE (M4 patch NNT tokenizer) ***\n")
