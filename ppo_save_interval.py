    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _save_checkpoint(ts, cs):
                ...
                if step % 1_000_000 == 0 and step >= 1_000_000:
                    suffix = f"{step // 1_000_000}M"
                elif step % 1_000 == 0 and step >= 1_000:
                    suffix = f"{step // 1_000}k"
                else:
                    suffix = f"{step}"

                dir_name = f"policy_{suffix}"
                orbax_checkpointer = PyTreeCheckpointer()
                options = CheckpointManagerOptions(max_to_keep=1, create=True)
                path = os.path.join(wandb.run.dir, dir_name)
                checkpoint_manager = CheckpointManager(path, orbax_checkpointer, options)
                print(f"Saving runner state to {path} at step {step}")
